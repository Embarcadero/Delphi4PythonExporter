...
uses
  System.SysUtils, System.IOUtils, System.Classes, FMX.Forms, Data.FMXForm;

procedure TFMXProducerTest.Setup;
begin
  FProducer := TProducerSimpleFactory.CreateProducer('FMX');
  FApplicationModel := BuildApplicationModel();
  FFormModel := BuildFormModel();
  FFormFileModel := BuildFormFileModel();
end;

procedure TFMXProducerTest.TearDown;
begin
  FFormFileModel.Free();
  FFormModel.Free();
...
function TFMXProducerTest.BuildApplicationModel: TApplicationProducerModel;
const
...
function TFMXProducerTest.BuildFormFileModel: TFormFileProducerModel;
begin
  Result := TFormFileProducerModel.Create();
  try
    Result.Directory := GetFilesDir();
    Result.FileName := 'Data.FMXForm';
...
function TFMXProducerTest.BuildFormModel: TFormProducerModel;
const
...
procedure TFMXProducerTest.CheckFormInheritance;
begin
  Assert.IsTrue(FProducer.IsValidFormInheritance(TForm));
end;

procedure TFMXProducerTest.GenerateApplication;
begin
...
//        main()
    ...
    Assert.IsTrue(LStrings[0] = 'from delphifmx import *');
    ...
    Assert.IsTrue(LStrings[6] = sIdentation1 + Format('Application.MainForm = %s(Application)', [
      FApplicationModel.MainForm]));
    Assert.IsTrue(LStrings[7] = sIdentation1 + 'Application.MainForm.Show()');
    Assert.IsTrue(LStrings[8] = sIdentation1 + 'Application.Run()');
    Assert.IsTrue(LStrings[9] = sIdentation1 + 'Application.MainForm.Destroy()');
...
procedure TFMXProducerTest.GenerateForm;
begin
  //Save the form file
...
//            pass
    ...
    Assert.IsTrue(LStrings[1] = 'from delphifmx import *');
    Assert.IsTrue(LStrings[2] = String.Empty);
    ...
    Assert.IsTrue(LStrings[7] = sIdentation2
      ...
      FFormModel.FileName, TFormFile('').AsPythonFmx()]));
...
procedure TFMXProducerTest.GenerateFormFileBin;
begin
  //Save the project file
  FProducer.SavePyFormFileBin(FFormFileModel, FFormFileModel.Stream);

  //Check for the generated file
  var LFilePath := TPath.Combine(FFormFileModel.Directory, FFormFileModel.FormFile.AsPythonFmx());
  Assert.IsTrue(TFile.Exists(LFilePath));
  ...
  try
    ...
    try
      var LForm := TFmxForm.CreateNew(nil);
      try
        try
...
procedure TFMXProducerTest.GenerateFormFileTxt;
begin
  //Save the project file
...
initialization
  TDUnitX.RegisterTestFixture(TFMXProducerTest);
